...
SpeculativeStats speculative_greedy_decode(std::vector<std::uint32_t>& context, std::size_t target_tokens,
  ...
  if (draft_n_max == 0) throw std::invalid_argument("speculative draft_n_max must be greater than zero");
  if (!draft || !verify) throw std::invalid_argument("speculative decoding requires draft and verify callbacks");
  ...
  while (stats.accepted_tokens + stats.rejected_tokens < target_tokens) {
    ...
    if (proposed.empty()) throw std::runtime_error("speculative draft callback returned no tokens");
    ...
    if (verified.empty()) throw std::runtime_error("speculative verify callback returned no tokens");
...
MtpStats mtp_greedy_decode(std::vector<std::uint32_t>& context, std::size_t target_tokens,
                           std::size_t draft_n_max, const MtpDraftFn& draft,
                           const MtpVerifyFn& verify, const MtpAcceptFn& accept) {
  return speculative_greedy_decode(context, target_tokens, draft_n_max, draft, verify, accept);
}
